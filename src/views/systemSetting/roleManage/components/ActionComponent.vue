<template>
  ...
  >
      ...
      <template v-else>
        ...
        >
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M23.6667 11.9993C23.6667 5.55603 18.4433 0.332682 12 0.332682C5.55669 0.332682 0.333344 5.55603 0.333344 11.9993C0.333344 18.4427 5.55669 23.666 12 23.666C18.4433 23.666 23.6667 18.4427 23.6667 11.9993ZM10.8333 16.8327C10.8333 17.385 11.2811 17.8327 11.8333 17.8327H12.1667C12.719 17.8327 13.1667 17.385 13.1667 16.8327V10.666C13.1667 10.1137 12.719 9.66602 12.1667 9.66602H11.8333C11.2811 9.66602 10.8333 10.1137 10.8333 10.666V16.8327ZM12 6.16602C12.6443 6.16602 13.1667 6.68835 13.1667 7.33268C13.1667 7.97701 12.6443 8.49935 12 8.49935C11.3557 8.49935 10.8333 7.97701 10.8333 7.33268C10.8333 6.68835 11.3557 6.16602 12 6.16602Z"
            fill="#D03050"
          />
        </svg>
      </template>
      <span style="margin-left: 12px">{{ typeObj[type] }}角色</span>
    </div>
    <div class="dialog-body">
      <div class="tips">
        {{
          type == "unPublish"
            ? "停用后该角色下员工账号将无法使用，确定停用？"
            : type == "publish"
            ? "启用后该角色下的员工账号可正常使用，确定启用？"
            : "删除后将回收该角色下的员工账号所有权限，确定删除？"
        }}
      </div>
    </div>
    <div class="popClass-bottom">
      ...
      <div
        class="common-btn publish-btn"
        v-else-if="type == 'publish'"
        @click="handlePublishArticle"
      >
        确定
      </div>
      <div class="common-btn delete-btn" v-else @click="handlePublishArticle">
        确定
      </div>
    </div>
  </el-dialog>
