      ...
      <div class="main-container">
        <div class="breadcrumb-box">
          ...
          <div class="breadcrumb-text">
            <span v-if="breadcrumbTextList.length == 1" style="color: #9EA4AA">{{ breadcrumbTextList.length > 0 ? breadcrumbTextList[0] : '' }}</span>
            <span v-else>
              <span v-for="(item, index) in breadcrumbTextList" :key="index">
                <span v-if="index < breadcrumbTextList.length - 1" style="color: #9EA4AA">{{ item }}</span>
    ...
    $route: {
      handler: function(route) {
        ...
        if(routeArr.length === 1) {
          this.breadcrumbTextList[0] = this.breadcrumbFirstRouteObj[routeArr[0]]
